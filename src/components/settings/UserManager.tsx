...
import { Plus, RefreshCw, Search, Users as UsersIcon, MoreHorizontal, Power, PowerOff, Shield, Eye, EyeOff, Loader2 } from 'lucide-react';
...
import { Profile, UserRole, AppRole, ROLE_LABELS } from '@/types/database';
...
export function UserManager() {
  ...
  const form = useForm<UserFormData>({
    ...
    defaultValues: { email: '', password: '', full_name: '', role: 'employee', phone: '', position: '' },
  });
  ...
  const changeUserRole = async (userId: string, newRole: AppRole) => {
    if (!entity) return;
    try {
      await supabase.from('user_roles').delete().eq('user_id', userId).eq('entity_id', entity.id);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => { form.reset(); setDialogOpen(true); }} className="bg-gradient-warm hover:opacity-90">
            ...
            <TableBody>
              {filteredUsers.map((user) => (
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <Badge variant="secondary" className="bg-primary/10 text-primary">{getUserRole(user)}</Badge>
                  </TableCell>
                  <TableCell>
                    <Badge className={user.profile.is_active ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'}>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        {availableRoles.map((r) => (
                          <DropdownMenuItem key={r.value} onClick={() => changeUserRole(user.id, r.value)}>
                            <Shield className="mr-2 h-4 w-4" />
                            Definir como {r.label}
                          </DropdownMenuItem>
                        ))}
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => toggleUserStatus(user.id, !user.profile.is_active)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-xl font-display">Novo Usuário</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleCreateUser)} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="position" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="Ex: Decorador" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
              <div className="flex justify-end gap-3 pt-4">
                ...
                <Button type="submit" disabled={isSubmitting} className="bg-gradient-warm hover:opacity-90">
